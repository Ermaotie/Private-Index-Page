...
function generateKeyBoard(keys, hash) {
    for (let index = 0; index < keys['length']; index = index + 1) { //0 1 2
        ...
        for (let index2 = 0; index2 < row['length']; index2 = index2 + 1) { //将固定的10换成可控制的
            let lcg_wrapper = tag('div');
            lcg_wrapper.className = 'lcg_wrapper';
            let lcg = tag('lcg');
            lcg.className = 'key';
            ...
            if (hash[row[index2]] === undefined) {
                lcg.setAttribute('title', '未设置网站导航');
            } else {
                lcg.setAttribute('title', hash[row[index2]]);
            }

            lcg.onclick = function (e) {
            ...
            lcg.appendChild(span);
            lcg.appendChild(img);
            lcg.appendChild(button);
            lcg_wrapper.appendChild(lcg);
            div.appendChild(lcg_wrapper);
        }
    }
}
